...
exports.login = async (req, res) => {
  try {
    ...
    const extractedName = email
      ? email.split("@")[0]
      : `user_${mobile || Math.floor(Math.random() * 10000)}`;
    ...
    let user = mobile
      ? await User.findOne({ phone: mobile })
...
exports.firebaseAuth = async (req, res) => {
  try {
    const { uid, phone, email, name, image_url, deviceid, fcm, versioncode } =
      req.body;
    ...
    if (!user) {
      ...
      console.log(
        "✅ New user created via Firebase auth:",
        user.email,
        "user_id:",
        user.user_id
      );
      ...
      await user.save();
      console.log(
        "✅ User data updated - Device ID:",
        user.deviceid,
        "FCM:",
        user.fcm?.substring(0, 20) + "..."
      );
    }
