...
exports.addSubscription = async (req, res) => {
  try {
    ...
    const newSubscription = new SubscriptionSchema({
      ...
      receipt: receipt || (razorpayOrder ? razorpayOrder.receipt : "Free"),
...
exports.updatePayment = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({
        message:
          "Missing payment parameters (order_id and payment_id required)",
        isSubscribed: false,
      });
    }
    ...
    console.log(
      "🔍 Looking for subscription with order_id:",
      razorpay_order_id
    );
    ...
    if (isTestPayment || isDevelopmentMode) {
      console.log(
        "⚠️  DEVELOPMENT/TEST MODE: Bypassing signature verification"
      );
      console.log(
        "Test payment:",
        isTestPayment,
      ...
      try {
        ...
        isValidSignature = verifyRazorpayPayment(
          razorpay_payment_id,
          razorpay_order_id,
          finalSignature
        );
        ...
        if (!isValidSignature && isRealRazorpayPayment) {
          console.log(
            "🔄 Primary signature failed, attempting payment status verification..."
          );
          ...
          try {
            ...
            if (paymentDetails.success && paymentDetails.payment) {
              ...
              if (
                ...
                console.log(
                  "✅ Payment verified via API despite signature mismatch"
                );
                isValidSignature = true;
              } else {
                console.log(
                  "❌ Payment API verification failed - order mismatch or invalid status"
                );
              }
            } else {
              console.log(
                "❌ Unable to fetch payment details from Razorpay API"
              );
            }
          } catch (apiError) {
            console.error(
              "❌ Payment API verification failed:",
              apiError.message
            );
    ...
    if (!isValidSignature) {
      ...
      return res.status(400).json({
        message:
          "Payment verification failed - invalid signature and unable to verify via API",
        isSubscribed: false,
        debug: {
          isTestPayment,
          isDevelopmentMode,
    ...
    console.log(`🔍 Payment processing decision:`, {
      ...
      willCallAPI:
        isRealRazorpayPayment || (!isTestPayment && !isDevelopmentMode),
    });
    ...
    if (isRealRazorpayPayment || (!isTestPayment && !isDevelopmentMode)) {
      try {
        ...
        const subscriptionAmount =
          existingSubscription.payment_info[0]?.amount ||
          existingSubscription.amount;
        const subscriptionCurrency =
          existingSubscription.payment_info[0]?.currency ||
        ...
        try {
          ...
          if (paymentDetails.success && paymentDetails.payment) {
            ...
            console.log(
              "⚠️  Could not verify payment status, allowing based on signature"
            );
            captureSuccessful = true;
      ...
      if (
        !captureSuccessful &&
        captureError &&
        captureError.includes("failed at gateway")
      ) {
        return res.status(400).json({
      ...
      console.log(
        "⚠️  DEVELOPMENT/TEST MODE: Skipping Razorpay API calls for:",
        razorpay_payment_id
      );
    ...
    return res.status(200).json({
      message:
        "Payment verified, captured and subscription activated successfully",
      isSubscribed: true,
      data: updated,
    });
  } catch (err) {
...
exports.deleteSubscription = async (req, res) => {
  try {
    const deletedSubscription = await SubscriptionSchema.findByIdAndDelete(
      req.params.id
    );
...
exports.processCashPayment = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        message:
          "Missing required parameters: user_id, channel_id, (video_id or plan_id), custom_duration, paid_amount",
        isSubscribed: false,
      });
    }
...
exports.manualRefundSubscription = async (req, res) => {
  try {
    ...
    if (
      ...
      !subscription.payment_info[0]?.razorpay_payment_id
    ) {
      return res.status(400).json({
